...
[Generator]
public class ClassProxyCreator : ISourceGenerator
{
    internal class SyntaxReceiver : ISyntaxReceiver
    {
        ...
        {
            if (syntaxNode is not ClassDeclarationSyntax interfaceDeclarationSyntax) { return; }
            ...
            {
                DecoratorRequestingClasses.Add(interfaceDeclarationSyntax);
            }
        }
    }
    ...
    public void Execute(GeneratorExecutionContext context)
    {
        if (context.SyntaxReceiver is not SyntaxReceiver receiver
            || !receiver.DecoratorRequestingClasses.Any()) { return; }

        foreach (var interfaceDeclarationSyntax in receiver.DecoratorRequestingClasses)
        {
            ...
            var interfaceType = namedTypeSymbol.Interfaces.First();
            var methods = interfaceType.GetMembers().OfType<IMethodSymbol>();
            ...
            foreach (var method in methods)
            {
                ...
                methodDefinitions.Add($$"""
                        ...
                        {
                            {{returnStatement}}await ipcHandler.ExecuteRemote<{{GetFullName(interfaceType)}}{{remoteReturnStatement}}>(plainlyRpcService
